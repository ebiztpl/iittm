                ...
                <table class="table table-bordered" id="myTable">
                  ...
                  <tbody>
                    ...
                    <tr ng-repeat='list in districtlist'>
                      <th style="background: #e4e4e4;">{{list.academic_intermediate}}</th>
                      <th style="background: #e4e4e4;" ng-click="statewisecity(list.id)">{{list.total}}</th>
                      <th style="background: #e4e4e4;">{{list.state}}</th>
                      <th style="background: #e4e4e4;">{{list.city}}</th>
      ...
      function validateCtrl($scope, $log, $http) {
        ...
        $scope.get_course = function($event) {
          ...
          $http({
            method: "get",
            dataType: 'html',
            url: "<?php echo site_url('admin/institute_wise_report_filter?data='); ?>" + data
        ...
        $scope.get_grand_total = function() {
          $http({
            method: "get",
            url: "<?php echo site_url('admin/institute_wise_report_total'); ?>"
          }).then(function(response) {
            $("#record").html(response.data.totalRecords);
          });
        };

        // Call it on page load
        $scope.get_grand_total();
