            ...
            <div class="box-body">
              ...
              <div class="row" style="margin-top: 10px;">
                ...
                <div class="col-sm-2">
                  ...
                  <input type="radio" class="candidate" name="candidate" value="1" checked> &nbsp;Admission &nbsp;&nbsp;&nbsp;
            ...
            <div class="box-body">
              ...
              <div class="row" style="margin-top:10px;">
                <div class="col-sm-2">
                  ...
                  <select name="exam_id" id="exam_id" class="form-control exam">
                ...
                <div class="col-sm-1">
                  <span class="badge badge-danger" style="line-height: 20px; width: 100%; display: grid;">
                    <input type="radio" value="present" id="present" name="attendance" class="exam" /> Present
                  </span>
                </div>

                <div class="col-sm-1">
                  <span class="badge badge-danger" style="line-height: 20px; width: 100%; display: grid;">
                    <input type="radio" value="absent" id="absent" name="attendance" class="exam" /> Absent
                  </span>
                </div>
                ...
                <div class="col-sm-1">
                  <span class="badge badge-danger" style="line-height: 20px; width: 100%; display: grid;">
                    <input type="radio" value="pass" id="pass" name="exam_status" class="exam" /> Pass
                  </span>
                </div>

                <div class="col-sm-1">
                  <span class="badge badge-danger" style="line-height: 20px; width: 100%; display: grid;">
                    <input type="radio" value="fail" id="fail" name="exam_status" class="exam" /> Fail
                  </span>
                </div>
...
<script>
  ...
  function Dashboard_Details($scope, $log, $http, $compile) {
    ...
    $scope.search_data_exam = function() {
      ...
      const exam_status = $("input[name='exam_status']:checked").val();
      const attendance = $("input[name='attendance']:checked").val();

      let whereClauses = [];

      if (exam_id) {
        whereClauses.push(`ce.exam_id = '${exam_id}'`);
      }

      if (exam_status) {
        whereClauses.push(`ce.exam_status = '${exam_status}'`);
      }

      if (attendance) {
        whereClauses.push(`cr.attendance = '${attendance}'`);
      }


      const where = whereClauses.length ? whereClauses.join(' AND ') : '';

      $('#item-list_wrapper').hide();
      $('#item-list-filter').show();

      if ($.fn.DataTable.isDataTable('#item-list-filter')) {
        $('#item-list-filter').DataTable().clear().destroy();
      }
      ...
      $('#item-list-filter').DataTable({
        ...
        ajax: {
          url: "<?php echo site_url('communication/filter_exam_candidate_search'); ?>",
          type: "POST",
          data: function(d) {
            d.filter_where = where;
            d.reg_status = 1;
          },
          dataSrc: function(json) {
            $("#loading").hide();
            $("#assignment_save").show();
            $("#record").html(json.recordsFiltered || 0);
            return json.data;
          },
