...
class Admin extends CI_Controller
{
	...
	public function bulkleaddata()
	{

		$bulklead_id = $this->uri->segment(3);
		$from = $this->input->post('from');
		$to = $this->input->post('to');

		$table = "user_master 
			LEFT JOIN candidate_data ON candidate_data.mobile_verified_id = user_master.user_id 
			LEFT JOIN bulk_leads ON candidate_data.source = bulk_leads.id
			LEFT JOIN states ON states.id = candidate_data.parma_state 
			LEFT JOIN cities ON cities.id = candidate_data.parma_city 
			LEFT JOIN states stc ON stc.id = candidate_data.corre_state 
			LEFT JOIN cities ctc ON ctc.id = candidate_data.corre_city";

		$fields = "*, 
			bulk_leads.title as bulk_title,
			states.name as parma_state, 
			cities.name as parma_city, 
			stc.name as corre_state, 
			ctc.name as corre_city";

		$conditions = ["user_master.login_status = 3", "candidate_data.duplicate = 0"];

		if (!empty($bulklead_id)) {
			$conditions[] = "candidate_data.source = " . $this->db->escape($bulklead_id);
		}
		...
		$whr = implode(" AND ", $conditions);

		$result = $this->db_lib->fetchRecords($table, $whr, $fields);
		$data['result']  = $result;
		$data['bulklead_id'] = $bulklead_id;


		$this->load->view($this->folder . "bulklead_data",  $data);
	}
	...
	public function filter_update()
	{
		...
		foreach ($query->result() as $r) {
			...
			$tranid = $user_data->razorpay_trans_id;
	...
	public function search_by_course()
	{
		...
		foreach ($query->result() as $candidate_data) {
			...
			if ($candidate_data != "") {
				...
				if ($candidate_data->course_name == 1) {
					$course = "BBA";
				}
				if ($candidate_data->course_name == 2) {
					$course = "MBA";
				}

				if ($candidate_data->course_name == 1) {
					$fullname = "" . $candidate_data->first_name . " " . $candidate_data->middle_name . " " . $candidate_data->last_name . "";
				}
	...
	public function defaule_tansactions()
	{
		...
		foreach ($query->result() as $candidate_data) {
			...
			if ($candidate_data->course_id == 1) {
				$course = "BBA";
			}
			if ($candidate_data->course_id == 2) {
				$course = "MBA";
			}
			...
			if (isset($raz_details['id']) != '') {
				$account = "<span style='color:green; font-weight:bold;'>IITTM</span>";
			} else {
				$account = "<span style='color:red; font-weight:bold;'>e-Biz</span>";
			}

			//print_r($raz_details);
			if (isset($raz_details['created_at']) != '') {
				$date = date("d-m-Y", $raz_details['created_at']);
			} else {
				$date = "";
			}
			if (isset($raz_details['amount']) != '') {
				$amt = $raz_details['amount'] / 100;
			} else {
				$amt = 0;
			}
			if (isset($raz_details['fee']) != '') {
				$fee = $raz_details['fee'] / 100;
			} else {
				$fee = 0;
			}
			if (isset($raz_details['tax']) != '') {
				$tax = $raz_details['tax'] / 100;
			} else {
				$tax = 0;
			}
			$data[] = array(
	...
	public function list_admission()
	{
		...
		foreach ($query->result() as $candidate_data) {
			...
			if ($candidate_data != "") {
				...
				if ($candidate_data->course_name == 1) {
					$fullname = "<a href='show_form_bba/$candidate_data->mobile_verified_id' target='_blank'>" . $candidate_data->first_name . " " . $candidate_data->middle_name . " " . $candidate_data->last_name . "</a>";
				}

				if ($candidate_data->course_name == 2) {
					$fullname = "<a href='show_form_mba/$candidate_data->mobile_verified_id' target='_blank'>" . $candidate_data->first_name . " " . $candidate_data->middle_name . " " . $candidate_data->last_name . "</a>";
				}
	...
	public function dashboard()
	{
		...
		$bulklead = $this->db
			...
			->join('user_master um', 'um.user_id = c.mobile_verified_id', 'left')
			->group_by('b.id')
			->where('c.duplicate = 0 and um.login_status=3')
